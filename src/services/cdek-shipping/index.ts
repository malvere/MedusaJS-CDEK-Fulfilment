...
import * as DataForRequest from './interface/requestData'
import * as DataForResponse from './interface/responseData'
...
export class Cdek_sdk {
	getTestServer(test: boolean) {
		request.getUrl(test)
	}
	async getToken(login: Login): Promise<string | any> {
        try {
	...
	async calculateOnTariffList(token: string, data: DataForRequest.TariffList): Promise<{ errors: DataForResponse.ErrorDetail[]; data: DataForResponse.TariffCodeList }> {
        const res: { errors: DataForResponse.ErrorDetail[]; data: DataForResponse.TariffCodeList } = { errors: [], data: { tariff_codes: [] } };
        ...
        try {
            const response = await axios.post('https://api.edu.cdek.ru/v2/calculator/tarifflist', data, {
                headers: {
            ...
            if (axios.isAxiosError(error) && error.response) {
                res.errors.push(...error.response.data.errors);
            } else {
                res.errors.push({ code: 'unknown', message: 'An unknown error occurred while making the API request.' });
            }
        }
